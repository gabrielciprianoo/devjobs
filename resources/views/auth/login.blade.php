    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between my-4">
            <label for="remember_me" class="inline-flex items-center">
                ...
                <span class="ms-2 text-sm text-gray-600">{{ __('Recordar Password') }}</span>
            </label>
        ...
        <div class="flex flex-col-reverse items-center gap-2 justify-between md:flex-row mt-4">
            @if (Route::has('password.request'))
                ...
                <a class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" href="{{ route('register') }}">
                    {{ __('¿No tienes una cuenta? Crear una') }}
                </a>
            @endif
